...
const getSubjectsForClass = (className: string) => {
    const classNumber = parseInt(className.replace('class-', ''), 10);
     if (classNumber >= 1 && classNumber <= 5) {
        return ["Hindi", "English", "Mathematics", "EVS (Environmental Studies)"];
    }
    if (classNumber >= 6 && classNumber <= 8) {
        return ["Hindi", "English", "Mathematics", "Science", "Social Science", "Sanskrit"];
    }
    if (classNumber >= 9 && classNumber <= 10) {
        return ["Hindi", "English", "Mathematics", "Science", "Social Science"];
    }
    if (classNumber === 11 || classNumber === 12) {
        return ["Physics", "Chemistry", "Mathematics", "Biology", "Computer Science", "English", "Accountancy", "Business Studies", "Economics", "History", "Geography", "Political Science"];
    }
  ...
  return (
    <div className="container mx-auto py-12 px-4 md:px-6">
      <div className="space-y-4 text-center mb-12">
        <h1 className="text-3xl md:text-4xl font-bold font-headline">Subjects for {classTitle} (KVS)</h1>
        <p className="text-muted-foreground md:text-xl">Complete study materials following the KVS curriculum</p>
      </div>
