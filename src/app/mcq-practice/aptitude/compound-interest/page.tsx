...
const examCategories = [
  { name: "Railway", icon: Train, href: "railway" },
  { name: "SSC", icon: Building, href: "ssc" },
  { name: "Banking", icon: Banknote, href: "banking" },
  { name: "OSSC", icon: PenTool, href: "ossc" },
  { name: "Mix", icon: Mix, href: "mix" },
];

export default function AptitudeTopicPage() {
  const topicName = "Compound Interest";
    ...
    <div className="container mx-auto py-12 px-4 md:px-6">
      ...
      <div className="max-w-4xl mx-auto grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {examCategories.map((category) => (
          <Link key={category.name} href={`/mcq-practice/aptitude/compound-interest/${category.href}`}>
