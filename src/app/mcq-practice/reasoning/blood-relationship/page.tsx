...
export default function BloodRelationshipPage() {
    ...
    const handleFinish = () => {
        ...
        for (let i = 0; i < mcqData.length; i++) {
            if (selectedAnswers[i] === mcqData[i].answer) {
                correctAnswers++;
            }
        }
    ...
    if (showScore) {
        return (
            <div className="container mx-auto p-4 md:p-6 flex justify-center items-center min-h-[calc(100vh-10rem)]">
                <Card className="w-full max-w-lg text-center">
                    <CardHeader>
                        <CardTitle>Quiz Completed!</CardTitle>
                        <CardDescription>Here is your score.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <p className="text-4xl font-bold">{score} / {mcqData.length}</p>
                    </CardContent>
                    <CardFooter>
    ...
    return (
        <div className="container mx-auto p-4 md:p-6">
            <h1 className="text-3xl font-bold mb-2">Blood Relationship MCQs</h1>
            <p className="text-muted-foreground mb-6">Question {currentQuestion + 1} of {mcqData.length}</p>

            <Card key={question.questionNumber}>
                <CardHeader>
                    <CardTitle>{question.type} - Question {question.questionNumber}</CardTitle>
                ...
                <CardContent>
                    ...
                    >
                        {question.options.map((option, index) => {
                            ...
                            return (
                                <div key={index} className={cn("flex items-center space-x-2 p-2 rounded-md", showAnswer && isCorrect && "bg-green-100 dark:bg-green-900 border border-green-500", showAnswer && isSelected && !isCorrect && "bg-red-100 dark:bg-red-900 border border-red-500")}>
                                    <RadioGroupItem value={option} id={`q${question.questionNumber}-o${index}`} />
                                    <Label htmlFor={`q${question.questionNumber}-o${index}`} className="flex-1 cursor-pointer">{option}</Label>
                                </div>
                            )
                        })}
                    </RadioGroup>
                    {showAnswer && <p className="mt-4 text-sm font-semibold">Correct Answer: {question.answer}</p>}
                </CardContent>
            ...
            <div className="flex justify-between items-center mt-6">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={handleEndQuiz} variant="destructive">End Quiz</Button>
                </div>
                <Button onClick={handleNext}>
                    {currentQuestion === mcqData.length - 1 ? "Finish" : "Next"}
                    {currentQuestion < mcqData.length - 1 && <ChevronRight className="ml-2 h-4 w-4" />}
                </Button>
            </div>
        </div>
    );
