...
const numbers = Array.from({ length: 21 }, (_, i) => i); // Numbers 0 to 20
...
export default function NumberPracticePage() {
  ...
  const [key, setKey] = useState(0); // Key to force re-render of canvas
  ...
  const handlePrevious = () => {
    setCurrentIndex((prevIndex) => (prevIndex > 0 ? prevIndex - 1 : numbers.length - 1));
    setKey(prevKey => prevKey + 1); // Change key to reset canvas
  };

  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex < numbers.length - 1 ? prevIndex + 1 : 0));
    setKey(prevKey => prevKey + 1); // Change key to reset canvas
  };

  return (
    <div className="relative w-full h-screen flex flex-col items-center justify-center bg-gradient-to-br from-green-200 to-cyan-200 dark:from-green-900 dark:to-cyan-900 overflow-hidden">
      ...
      <div className="absolute top-20 right-20 w-48 h-48 bg-red-300 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-2000"></div>
      <div className="absolute bottom-10 left-20 w-24 h-24 bg-blue-300 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-4000"></div>
      ...
      <DrawingCanvas key={key} ref={canvasRef} lineColor={drawingColor} />
      ...
      <Button onClick={handlePrevious} size="lg" className="absolute left-5 top-1/2 -translate-y-1/2 z-20 rounded-full h-16 w-16 md:h-20 md:w-20 p-0 shadow-lg bg-white/50 hover:bg-white/80 text-gray-800 backdrop-blur-md">
        ...
        <span className="sr-only">Previous Number</span>
      </Button>
      <Button onClick={handleNext} size="lg" className="absolute right-5 top-1/2 -translate-y-1/2 z-20 rounded-full h-16 w-16 md:h-20 md:w-20 p-0 shadow-lg bg-white/50 hover:bg-white/80 text-gray-800 backdrop-blur-md">
        ...
        <span className="sr-only">Next Number</span>
      </Button>
